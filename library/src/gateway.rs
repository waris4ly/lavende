            ...
            async fn run(&mut self, encoder: &mut Encoder) -> Result<(), GatewayError> {
                ...
                while !self.config.cancel_token.is_cancelled() {
                    ...
                    if self
                        ...
                        .is_multiple_of(100)
                    {
                        self.config.persistent_state.lock().await.rtp_state =
                            Some(self.transport.rtp);
                    }
                }
            ...
            async fn tick(
                ...
                while loop_count < 10 {
                    ...
                    if has_input {
                        ...
                        if self.last_tx_time.elapsed()
                            >= Duration::from_millis(UDP_KEEPALIVE_GAP_MS)
                        {
        ...
        impl<'a> SessionState<'a> {
            ...
            pub async fn handle_text(&mut self, text: String) -> Option<SessionOutcome> {
                ...
                match op {
                    ...
                    OpCode::DavePrepareTransition => {
                        self.on_dave_prepare_transition(payload.d).await
                    }
                    OpCode::DaveExecuteTransition => {
                        self.on_dave_execute_transition(payload.d).await
                    }
            ...
            pub async fn handle_binary(&mut self, bin: Vec<u8>) {
                ...
                match op {
                    ...
                    27 => match dave.process_proposals(data) {
            ...
            async fn on_ready(&mut self, d: Value) -> Option<SessionOutcome> {
                ...
                if self.gateway.channel_id.0 > 0 {
                    let ver = d["dave_protocol_version"]
                        .as_u64()
                        .unwrap_or(DAVE_INITIAL_VERSION as u64)
                        as u16;
                    let mut dave = self.dave.lock().await;
                    if ver == 0 {
                        dave.reset();
...
pub mod udp_link {
    use crate::{
        ...
        gateway::{
            constants::{
                RTP_OPUS_PAYLOAD_TYPE, RTP_TIMESTAMP_STEP, RTP_VERSION_BYTE,
                UDP_PACKET_BUF_CAPACITY,
            },
    ...
    impl UDPVoiceTransport {
        pub fn new(
            ...
            let crypto = match mode {
                ...
                _ => CryptoBackend::XSalsa20Poly1305(Box::new(XSalsa20Poly1305::new(
                    &secret_key.into(),
                ))),
            };
            Ok(Self {
